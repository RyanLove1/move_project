...
urlpatterns = [
    ...
    url('^feedback/', views.FeedbackView.as_view(), name='feedback'),  # 反馈建议
    url('^(?P<pk>\d+)/collect_videos/', views.CollectListView.as_view(), name='collect_videos'),  # 收藏视频
    url('^(?P<pk>\d+)/like_videos/', views.LikeListView.as_view(), name='like_videos'),  # 喜欢
]
